...
// Get all sections
$sections_stmt = $conn->prepare("SELECT id, section_name FROM section");
...
// Handle DELETE - admin can delete any activity
...
// Handle form submission (adding activities)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $num_questions = count($_POST['question_text']);
    $section_id = intval($_POST['section_id'] ?? 0);

    // No need to check section assignment for admin
    ...
    // For admin, teacher_id can be 0 or NULL (depends on your design)
    $creator_id = 0; 
    ...
    for ($i = 0; $i < $num_questions; $i++) {
        ...
        $stmt->bind_param("isssssis", $creator_id, $type, $question, $choices_json, $correct, $now, $section_id, $title);
...
// Fetch all activities with section name
$stmt = $conn->prepare("
    SELECT a.*, s.section_name, t.email as teacher_email 
    FROM activities a 
